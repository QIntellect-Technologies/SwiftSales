...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans bg-slate-50 text-slate-800 selection:bg-blue-100 selection:text-blue-900">
      ...
      >
        <div
          className={`mx-auto max-w-7xl transition-all duration-500 border shadow-[0_8px_30px_rgba(0,0,0,0.06)] rounded-full flex items-center justify-between px-6 pr-2 ${scrolled
            ? 'bg-white border-slate-100 h-16'
            : 'bg-white border-slate-100 h-20'
            }`}
        >
          {/* Official Logo Integration */}
          <NavLink to="/" className="flex items-center gap-3 group mr-8 pl-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                />
                <span className="text-2xl font-bold text-white tracking-tight">Swift Sales</span>
              </div>
